...
#' @export
get_reads_df <- function(location, postprocess=F, thresh=500) {
    ...
    #    only have the desired 3 columns.
    bin_df <- read.csv(location, sep = " ", header = FALSE,
        stringsAsFactors = F)
    if ( ncol(bin_df) > 3 ) {
    ...
    # Assuming this is a properly structed processed coverage file (which it
    #    has to be to have gotten this far), the binwidth is equal to the
    #    difference in start location of any 2 bins on the same chromosome.
    # Picking bins 1 & 2 here as we know they have to be on the same chromosome.
    binsize <- bin_df[2, "start_idx"] - bin_df[1, "start_idx"]

    # Vivian's code needs a chunk of preprocessing. (normalized count,
    #        thresholded count)
    if (postprocess) {
        bin_df$end_idx <- bin_df$start_idx + binsize - 1

        bin_df$norm_count <- bin_df$count / sum(as.numeric(bin_df$count),
            na.rm = T) * 1e6
...
#' @export
get_reads_pvals_df <- function(location) {
    # TODO : fail neatly if given an unfitted coverage (no p-value)
    bin_df <- read.csv(location, sep = " ", header = FALSE,
        stringsAsFactors = F,
        col.names = c("chr", "start_idx", "count", "p_value"))
    return(bin_df)
}
...
#' @export
get_parameters <- function(location) {
    params_loc <- paste0(location, ".PARAMS")
    if (!file.exists(params.loc)) {
        # TODO : Is there a better way of throwing an error?
        print(paste0("Parameters missing. Please ensure parameters",
            " are saved in ", params_loc, "."))
        return(NA)
    }
    params_df <- read.csv(params_loc, sep = ",", header = T,
        stringsAsFactors = F)
    return(params_df)
}
...
#' @param pre_filter Filter dataframe before returning, requiring a minimum score of mappability_threshold
#' @param mappabiity_threshold (default:0.9) if pre_filter is T, filter dataframes to only rows above or equal to this threshold.
#' @return dataframe with 3 columns, chr, start, & score
#' @export
get_mappability <- function(location="/reference/mappability_5k.bed",
    pre_filter = F, mappability_threshold = 0.9) {

    map_df <- read.table(file = location, sep = "\t",
        col.names = c("chr", "start", "score"), header = FALSE,
