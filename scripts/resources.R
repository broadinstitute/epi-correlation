...
#' @export
GetReadsPvalsDF <- function(location) {
  # TODO : fail neatly if given an unfitted coverage (no p-value)
  bin_df <- read.csv(
    location, sep = " ", header = FALSE,
    stringsAsFactors = F,
...
#' @export
GetMappability <- function(location="/reference/hg19/mappability_5k.bed",
  pre_filter = F, mappability_threshold = 0.9) {

  map_df <- read.table(
    file = location, sep = "\t",
    col.names = c("chr", "start", "score"),
    header = FALSE,
