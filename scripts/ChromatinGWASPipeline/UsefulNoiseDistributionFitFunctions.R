...
GetGammaFromMuSigma <- function(sigma, mu, var_in, fun_str,
    lower_tail = FALSE, show_output = FALSE){
...
GetTruncDistribution <- function(params_df, n_points, threshold){
  ...
  mu0 <- params_df$mu0
  gamma_sim <- GetGammaFromMuSigma(
    sigma = sigma0, mu = mu0, var_in = n_points, 
    fun_str = "rgamma", show_output = FALSE
    )
  # then remove all p_values below a threshold
  # replace with qgamma
  x_thresh <- GetGammaFromMuSigma(
    sigma = sigma0, mu = mu0, var_in = threshold, 
...
GetLoessModel <- function(input_df, sigma_trunc_str = "sigma_0.2",
    mu_trunc_str = "mu_0.2", use_subset = FALSE){
  # use formula to calculate mu, sigma for a given pair of mu_trunc and sigma_trunc
  mu_formula <- as.formula(
    paste("mu0", "~", mu_trunc_str, "+", sigma_trunc_str)
    )
  sigma_formula <- as.formula(
    paste("sigma0", "~", mu_trunc_str, "+", sigma_trunc_str)
    )
  if (use_subset){
    train_idx <- sample(
      x = dim(input_df)[1],
      size = floor(dim(input_df)[1] / 3)
      )
    mu_model <- loess(
      formula = mu_formula,
      data = input_df,
...
InitializeRecalculateIdx <- function(working_df, step_size){
  stats_df0 <- working_df %>% dplyr::filter(norm_count > 0) %>%
    summarise(mu0 = mean(norm_count), sigma0 = sd(norm_count))
  px0 <- GetGammaFromMuSigma(
    sigma = stats_df0$sigma0,
    mu = stats_df0$mu0,
    var_in = working_df$norm_count,
    fun_str = "pgamma"
    )
...
PValueIterator <- function(working_df, step_size = 0.01,
    max_iter = 15, tolerance = 0.5, use_model = FALSE, ...){

  list_args <- list(...)
  if ("input_df" %in% names(list_args)){
    input_df <- list_args$input_df
    use_model <- TRUE
    sigma_trunc_str <- paste0("sigma_", step_size)
    mu_trunc_str <- paste0("mu_", step_size)
    # model_list has sigma_model and mu_model
    model_list <- GetLoessModel(
      input_df = input_df,
      sigma_trunc_str = sigma_trunc_str,
      mu_trunc_str = mu_trunc_str
      )
  } 
  ...
  params_df <- data.frame(
    ...
    k = numeric(max_iter),
    mean = numeric(max_iter),
    sd = numeric(max_iter),
    px = numeric(max_iter)
    )
  StopCriterion <- function(mean1, mean0){
    abs((mean1 - mean0)/mean0) * 100 < tolerance
    }
  # now iterate
  while (num_iter <= max_iter){
    ...
    if (use_model){
      ...
      params_list <- llply(
        .data = model_list,
        .fun = PredictValuesFromLoessModel,
        "newdata" = stats_df
        )
      mu_count <- params_list$mu_model
      sigma_count <- params_list$sigma_model
      beta <- mu_count/sigma_count^2
      k <- beta*mu_count
      px <- GetGammaFromMuSigma(
        sigma = sigma_count,
        mu = mu_count,
        var_in = working_df$norm_count,
        fun_str = "pgamma"
      ...
      mu_count <- stats_df$mu_count
      sigma_count <- stats_df$sigma_count
      px <- GetGammaFromMuSigma(
        sigma = sigma_count,
        mu = mu_count, var_in = working_df$norm_count, fun_str = "pgamma")
    }
    # collect parameters
    params_df$beta[num_iter] <- mu_count / sigma_count^2
    params_df$k[num_iter] <- params_df$beta[num_iter] * mu_count
    params_df$mu[num_iter] <- mu_count
    params_df$sigma[num_iter] <- sigma_count
...
GetPValueHistogram <- function(params_df, subtitle_str = "Iteration_no",
    title_str = ""){
  ...
  p_hist <- ggplot(px_melted, aes(x = P_value, y = ..density..)) + 
    ...
    facet_wrap(as.formula(paste0("~", subtitle_str) )) + 
    ggtitle(label = title_str) +
    theme(text = element_text(size = 16)
    )
  p_dens <- ggplot(px_melted, aes(x = P_value)) + 
    geom_density(aes_string(color = subtitle_str)) +
...
GetIterationHistogram <- function(params_df, working_df,
    has_peak = TRUE, bin_width = 0.1, x_lim = 25){
  ...
  getIterDf <- function(params_df_subset, norm_count, has_peak = NA){
    ...
    iter_df_subset <- data.frame(
      norm_count = numeric(df_len),
      sim_vals = numeric(df_len)
      )
    iter_df_subset$norm_count <- norm_count
    iter_df_subset$name <- paste0(
      "Beta = ", toString(round(params_df_subset$beta, 3)), 
      ", k = ", toString(round(params_df_subset$k, 3)),
      "\n Mean = ", toString(round(params_df_subset$mu, 1)), 
      ", Sd = ", toString(round(params_df_subset$sigma, 0))
      )
    iter_df_subset$sim_vals <- rgamma(
      n = df_len,
      shape = params_df_subset$k,
      rate = params_df_subset$beta
      )
    if (length(has_peak) > 1){
      iter_df_subset$has_peak <- has_peak
      }
  ...
  iter_df_all <- dlply(
    ...
    .fun = getIterDf,
    .variables = "iter_num",
    "norm_count" = working_df$norm_count, 
  ...
  iter_hist <- ggplot(iter_df_melted, aes(x = norm_count, y = ..density..)) + 
    geom_histogram(aes(fill = has_peak), binwidth = bin_width,
      position = "identity", alpha = 0.5) +
    geom_density(aes(x = sim_vals), trim = TRUE) + 
    facet_wrap(~ name) +
    theme(text = element_text(size = 16)) +
    ggtitle(params_df$Name[1]) +
...
GetComparisonHistogram <- function(bin_df_all, params_df_all,
    has_peak = TRUE, bin_width = 10, max_dens = 500, min_dens = 0,
  ...
  GetSimulatedDistribution <- function(params_df_converged, npoints = 512){
    ...
    sim_distribution$sim_values <- dgamma(
      x = seq(0, max_dens, length.out = npoints),
      shape = params_df_converged$k, 
      rate = params_df_converged$beta
      )
    if ("lambda" %in% names(params_df_converged)){
      sim_distribution$sim_values <- sim_distribution$sim_values * 
        params_df_converged$lambda
      if (has_peak){
        sim_distribution$sim_values <- sim_distribution$sim_values / 
          params_df_converged$no_peak
      }
    }
    sim_distribution$x_values <- seq(0, max_dens, length.out = npoints)
    sim_distribution$Name <- params_df_converged$Name[1]
    sim_distribution$Name_full <- paste(
      params_df_converged$Name,
      "k =",
      round(params_df_converged$k,1)
      )
  ...
  test_sim <- dlply(
    params_df_converged,
    .(Name),
    GetSimulatedDistribution,
    npoints = 10000
    )
  test_sim_melted <- do.call("rbind", test_sim)
  if (!is.null(factor_levels)){
    bin_df_melted$Name <- factor(
      x = bin_df_melted$Name,
      levels = factor_levels
      )
  } else {
    bin_df_melted$Name <- factor(
      x = bin_df_melted$Name,
      levels = unique(bin_df_melted$Name)
      ) 
  }
  test_sim_melted$Name <- factor(
    test_sim_melted$Name,
    levels = levels(bin_df_melted$Name)
    )
  fit_plot <- ggplot(
    bin_df_melted %>% dplyr::filter(thresh_count > min_dens),
    aes(x = thresh_count)
    )
  params_text <- paste(
    params_df_converged$Name,
    round(params_df_converged$k, 2)
    )
  if (has_peak){
    fit_plot <- fit_plot + 
      geom_histogram(
        aes(y = ..density.., fill = has_peak),
        alpha= 0.5,
        binwidth = bin_width,
        position = "identity"
        )
  ...
  fit_plot <- fit_plot + 
    geom_line(
      data = test_sim_melted,
      aes(x = x_values, y = sim_values, color = "Sim fit"),
      color = "black") + 
...
GetPValueComparisonHistogram <- function(params_df_all, bin_df_all,
    bin_width = 0.05, factor_levels = NULL){
  ...
  bin_df_names <- laply(
    .data = bin_df_all,
    .fun = function(bin_df){bin_df$Name[1]}
    )
...
GetPValueLevelHistogram <- function(working_df, params_df){
  ...
  px_melted$norm_count <- rep(
    x = working_df$norm_count,
    times = dim(params_df)[1]
    )
...
RunAllCalculations <- function(bin_df_all, use_model, input_df){
  ...
  params_df_melted <- do.call("rbind", params_df_all)
  ggplot(params_df_melted, aes(x = rep(1:12, 10), y = ks_value)) +
...
GetComparisonQQPlot <- function(bin_df_all, params_df_all){
  ...
  params_df_converged <- params_df_melted %>% group_by(Name) %>% 
    dplyr::summarize(beta = dplyr::last(beta), k = dplyr::last(k))
  GetSimulatedQuantiles <- function(
        params_df_converged, bin_df,
        p_value_thresh = 0.01
        ){
    norm_count <- bin_df %>%
      dplyr::filter(Name == params_df_converged$Name[1]) %>%
    ...
    sim_distribution$sim_values <- qgamma(
      p = x_values, 
      shape = params_df_converged$k, 
      rate = params_df_converged$beta,
      lower.tail = FALSE
      )
    sim_distribution$p_thresh <- qgamma(
      p = p_value_thresh,
      shape = params_df_converged$k,
      rate = params_df_converged$beta,
      lower.tail = FALSE
    ...
    sim_distribution$Name <- params_df_converged$Name[1]
    sim_distribution$norm_count <- sort(
  ...
  qq_sim <- dlply(
    params_df_converged,
    .(Name),
    GetSimulatedQuantiles, "bin_df" = bin_df_melted)
  qq_sim_melted <- do.call("rbind", qq_sim)
  qq_plot <- ggplot(qq_sim_melted, aes(x = sim_values, y = norm_count)) +
    geom_point() + 
