...
GetSaveFilename <- function(save_fields, joined_df, save_location, APP_full){
  if(save_location == 'default'){
    bed_file_location <- joined_df %>% dplyr::filter(Name == APP_full) %>% dplyr::select(Binned.Filename)
    save_location <- dirname(path = toString(bed_file_location$Binned.Filename))
  }
...
ProcessSingleFile <- function(APP_full, joined_df, reference_df, use_segmentation = FALSE, step_size = 0.01, 
                              save_location = 'default', save_fields, p_value_thresh = 0.01, return_params = FALSE, 
                              ...
                              estimation_method = 'cvm', overwrite_file = FALSE, reference_file="/seq/epiprod02/Polina/IGVCount/5k/bin_bl.bed"){
  ...
  if(file_info$size == 0){
    print(paste(APP_full, 'improperly binned'))
    return(FALSE)
  ...
  save_filename <- GetSaveFilename(save_fields, joined_df, save_location, APP_full)
  if(file.exists(save_filename) & !overwrite_file){
    print(paste(APP_full, 'already processed'))
    return(FALSE)
  }
  bin_df <- GetBinDf(APP_full = APP_full, joined_df = joined_df, reference_df = reference_df, 
                     use_segmentation = use_segmentation, use_wanted_chr = FALSE, bin_size = 5000, reference_file = reference_file)
  if(is.null(bin_df)){
    print(paste(APP_full, 'does not have the appropriate number of rows or columns.'))
    return(FALSE)
  }
  # get p values
  if(estimation_method == 'iteration'){
    ...
    if(!is.null(mappability_threshold)){
      map_df <- read.table(file = map_df_filename, sep = map_df_sep, col.names = c('chr', 'start_idx', 'score'), header = FALSE)
  ...
  if(save_params){
    params_filename <- GetSaveFilename(save_fields = 'params_df', joined_df = joined_df, save_location = save_location, 
                                      APP_full = APP_full)
...
ProcessManyFiles <- function(joined_df_filename = '/seq/epiprod02/Polina/IGVCount/SANDBOX/5k/Combined_APP_Segmentation.csv', 
                             joined_df_sep_str = '\t',
                             ...
                             save_fields = c('chr', 'p_value'), save_location = 'default', return_params = FALSE,
                             bin_size = 5000, map_df_filename = 'MappabilityTrack/binned_mappability_scores_5k.txt',
                             map_df_sep = '\t', mappability_threshold = 0.9, exclude_X_Y = TRUE){
  joined_df <- read.csv(file = joined_df_filename, header = TRUE, sep = joined_df_sep_str, stringsAsFactors = FALSE)
  reference_df <- read.table(file = reference_df_filename, header = FALSE, sep = reference_df_sep_str,
                              stringsAsFactors = FALSE)
  # check that all wanted_APP are in joined_df
  missing_APP <- sum(!(wanted_APP %in% joined_df$Name))
  if(missing_APP > 0){
    print(paste('Missing APP: ', wanted_APP[!(wanted_APP %in% joined_df$Name)], sep = '\n'))
  ...
  for(APP in APP_list){
    tracker[[counter]] <- ProcessSingleFile(APP_full = APP, joined_df = joined_df, reference_df = reference_df, 
    ...
    if(counter%%10 == 0){print(paste('Iteration', counter))}
    counter <- counter + 1
  }
  return(tracker)
}
